...
namespace API.Utilites
{
    public class SongUtilDatabase : ISongUtilities
    {
        ...
        { 
            ...
            WriteToDatabase.WriteAllToDatabase(playlist); // write new values to database
            //WriteToFile.WriteAllToFile(playlist);
        }
        ...
        public void EditSong() //updated edit song method
        {
            string userInp;
            int inp;
            int ind;
            do
            {
                do
                {
                    Console.Clear();
                    PrintPlaylist();
                    Console.WriteLine("Please select the ID of the song title you would like to edit:");
                    userInp = Console.ReadLine();
                    inp = int.Parse(userInp);
                } while(!CheckValidInput(userInp)); //checks to make sure it is valid input

                ind = playlist.FindIndex(currentSong => currentSong.SongID == inp);
            } while (!CheckValidIndex(ind));
            
            string title; //new user created title
            Console.WriteLine("Enter the new edited title for the song:");
            title = Console.ReadLine();
            playlist[ind].SongTitle = title;
